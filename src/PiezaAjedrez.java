...
public abstract class PiezaAjedrez extends Pieza {
	 ...
	 */
	protected PiezaAjedrez (Jugador jugador, TipoPiezaAjedrez tipoPieza, int fila, char columna) {
		super(jugador, 8-fila, (columna - 'a'));
	...
	public final boolean movimientoDentroLimites (int toFila, int toColumna) {
		return toFila >= 0 && toFila < 8 && toColumna >= 0 && toColumna < 8;
	}
	...
	public final boolean puedeMover (int toFila, char toColumna){
		return this.movimientoDentroLimites(toFila - 1, (int)(toColumna - 'a')) && 
				this.getFila() != toFila - 1 && this.getColumna() != (int)(toColumna - 'a');
	}
...
//	métodos auxiliares
	...
	protected int distManhattan(int filaNueva, int columnaNueva) {
		return (Math.abs(this.getFila() - filaNueva) + Math.abs(this.getColumna() - columnaNueva));
	}
	
	/*protected void casillasVisitables (IList<Casilla> resultado, int incFila, int incColumna){
		int prevSize = resultado.size() - 1; //comprobamos si se ha añadido algún elemento nuevo comparando el tamaño de la lista en esta 
		//iteración con el tamaño en la iteración anterior
		int fila = this.getFila() + incFila;
		int columna = this.getColumna() + incColumna;
		PiezaAjedrez pieza = queHay(fila, columna);

		//al separar en casos añadimos complejidad al introducir tres bloques if, else-if, else 
		//pero la reducimos en dos de los casos de O(n^2) a O(n)
		
		if(incFila != 0 && incColumna == 0) { //solo se desplaza la fila
			for(int i = fila; movimientoDentroLimites(i, columna) && prevSize < resultado.size(); i += incFila, prevSize++) {
				pieza = queHay(i, columna);
				if(pieza == null) {
					resultado.add(resultado.size(), new Casilla (i, columna));
					
				} else {
					if(pieza.getJugador().getId() != this.getJugador().getId()) {
						resultado.add(resultado.size(), new Casilla (i, columna));
						prevSize++; // así no cumplirá la condición del for la próxima vez y reutilizamos esta variable en vez de crear una boleana
					} // if
				} //else
			} //for
			
		} else if(incFila == 0 && incColumna != 0) { //solo se desplaza la columna
			for(int j = columna; movimientoDentroLimites(fila, j) && prevSize < resultado.size(); j += incColumna, prevSize++) {
				pieza = queHay(fila, j);
				if(pieza == null) {
					resultado.add(resultado.size(), new Casilla (fila, j));
			...
			for(int i = fila; prevSize < resultado.size(); i += incFila, prevSize++) {
				for(int j = columna; movimientoDentroLimites(i, j) && prevSize < resultado.size(); j += incColumna, prevSize++) {
					pieza = queHay(i, j);
					if(pieza == null) {
	...
	protected void casillasVisitables (IList<Casilla> resultado, int incFila, int incColumna){
		...
		while(continuacion && movimientoDentroLimites(fila, columna)) {
			PiezaAjedrez pieza = queHay (fila, columna);
		...
		do {
			...
			if(movimientoDentroLimites(fila, columna) && pieza == null) {
				resultado.add(resultado.size(), new Casilla (fila, columna));
				fila += incFila;
				columna += incColumna;
				continuar = true;
				...
				if(movimientoDentroLimites(fila, columna) && pieza.getJugador().getId() != this.getJugador().getId()) {
					resultado.add(resultado.size(), new Casilla (fila, columna));
				}
				continuar = false;
			}
	 ...
	 * @param columnaEst columna de la casilla que se desea estduiar
	 */
	protected void movimientoDentroLimites(IList<Casilla> resultado, int filaEst, int columnaEst) {
